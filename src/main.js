...
// ─── CARDS ───────────────────────────────────────────────────────────────────
const TLoader = new TextureLoader(LoadManager);
const cards = [];
...
function scheduleSnap() {
  ...
  restTimer = setTimeout(() => {
    ...
    const idx =
      ((Math.round(snapped / STRIDE) % CONFIG.cardCount) + CONFIG.cardCount) %
      CONFIG.cardCount;
...
function animate() {
  ...
  const rawNorm = THREE.MathUtils.clamp(
    rawInputDelta / CONFIG.maxScrollVelocity,
    0,
    1,
  );
  ...
  smoothActive += (activeTarget - smoothActive) * 0.15;
  ...
  rgbPass.uniforms.uMouse.value.lerp(targetMouse, 0.2);
  ...
  for (let i = 0; i < CONFIG.cardCount; i++) {
    ...
    mat.uniforms.uScrollVelocity.value =
      scrollVelocity / CONFIG.maxScrollVelocity;
